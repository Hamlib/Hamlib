...
static int flir_open(ROT *rot)
{
    ...
    if (rot->caps->rot_model == ROT_MODEL_FLIR)
    {
        simulating = 1;
        rig_debug(RIG_DEBUG_VERBOSE, "%s: flir rotator so simulating speed\n",
                  __func__);
    }
...
static void flir_simulate_rotation(ROT *rot)
{
    struct flir_priv_data *priv = (struct flir_priv_data *)
                                       rot->state.priv;
    struct timeval tv;
