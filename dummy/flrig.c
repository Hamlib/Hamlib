...
static int read_transaction(RIG *rig, char *xml, int xml_len)
{
    ...
    rs->rigport.timeout = 4000; // 3 second read string timeout
	...
	// Testing slowing down 3 of commands per second
	usleep(2*1000);
        retval = RIG_OK;
    }
    else {
...
static int flrig_set_split_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width)
{
    ...
    retval = flrig_set_mode(rig,RIG_VFO_B,mode,width);
...
static int flrig_set_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width)
{
    ...
    if (vfo == RIG_VFO_B && priv->curr_vfo != RIG_VFO_B) {
        vfoSwitched = 1;
    ...
    char xml[MAXXMLLEN];
    char *pxml = xml_build("rig.set_mode", cmd_buf, xml, sizeof(xml));
    ...
    // Need to update the available bandwidths for this mode
    if (width > 0) {
...
static int flrig_get_mode(RIG *rig, vfo_t vfo, rmode_t *mode, pbwidth_t *width)
{
    ...
    int vfoSwitched=0;
    if (vfo == RIG_VFO_B && curr_vfo != RIG_VFO_B) {
        vfoSwitched = 1;
    }
    ...
    char xml[MAXXMLLEN];
    char *pxml = xml_build("rig.get_mode", NULL, xml, sizeof(xml));
    retval = write_transaction(rig, pxml, strlen(pxml));
    if (retval < 0) {
        return retval;
    }
    ...
    *mode = retval;
    ...
    // Get the bandwidth
    pxml = xml_build("rig.get_bw", NULL, xml, sizeof(xml));
    retval = write_transaction(rig, pxml, strlen(pxml));
    if (retval < 0) {
        return retval;
    }
